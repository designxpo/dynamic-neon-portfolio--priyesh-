...
const SkillsForm: React.FC = () => {
    ...
    if (isLoading) return (<div className="flex items-center justify-center py-12"><div className="text-gray-400">Loading skills...</div></div>);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex justify-between items-center">
                ...
                <button onClick={handleAddNew} className="admin-button-primary flex items-center gap-2"><Plus size={20} />Add New Skill</button>
            </div>
                ...
                {skills.map((skill) => (
                    <div key={skill.id} className="admin-card group">
                        <div className="flex flex-col items-center text-center gap-3">
                            ...
                            <div className="flex items-center gap-2">
                                {skill.image?.url ? (
                                    ...
                                    <span className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-white/5 text-gray-400 border border-white/10">Default Icon</span>
                                )}
                            </div>
                        </div>
                        <div className="flex items-center justify-center gap-2 mt-4 pt-4 border-t border-white/10 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => handleEdit(skill)} className="admin-icon-button" title="Edit"><Edit2 size={14} /></button>
                            <button onClick={() => handleDelete(skill.id)} className="admin-button-danger p-1.5" title="Delete"><Trash2 size={14} /></button>
                        </div>
                    </div>
                ))}
            </div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="admin-card">
                            ...
                            <div className="flex gap-3">
                                ...
                                {currentItem.image?.url && (<button type="button" onClick={() => setCurrentItem(p => p ? {...p, image: { url: '', alternativeText: '' }} : null)} className="admin-button-danger">Remove</button>)}
                            </div>
                            <p className="text-xs text-gray-500 mt-2">Recommended size: 64x64 pixels</p>
                        </div>
                        <div className="flex justify-end gap-3 pt-4 border-t border-white/10"><button onClick={() => setIsModalOpen(false)} className="admin-button-secondary">Cancel</button><button onClick={handleSave} disabled={saving} className="admin-button-primary">{saving ? 'Saving...' : 'Save Skill'}</button></div>
                    </div>
                )}
            </Modal>
        </div>
